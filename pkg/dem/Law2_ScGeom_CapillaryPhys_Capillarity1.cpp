...
void Law2_ScGeom_CapillaryPhys_Capillarity1::action()
{
	...
	for (; ii!=iiEnd ; ++ii) {
		...
		if ((*ii)->isReal()) {
			...
			Real liquidTension = 0.000073; 	// superficial water tension at 20 Celsius degrees for particles of 1m size, this is not the real value but the value chosen to solve numerically correctly the equation
...
// 				solution(Pinterpol? capillary->Interpolate(R1,R2,Dinterpol, Pinterpol, currentIndexes) : MeniscusParameters());
				...
				/// meniscus volume
				Real Vinterpol = solution.volume;
				if (!hertzOn) {
